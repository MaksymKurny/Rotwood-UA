-- TODO(mods): This is an old DST mod file and might not be used by Rotwood. Needs cleanup and move to mods/ folder.
...
function AreServerModsEnabled()
	if ModManager == nil then
		TheLog.ch.Mods:print("AreServerModsEnabled returning false because ModManager hasn't been created yet.")
		return false
	end
...
function AreAnyModsEnabled()
	if ModManager == nil then
		TheLog.ch.Mods:print("AreAnyModsEnabled returning false because ModManager hasn't been created yet.")
		return false
	end
...
function AreAnyClientModsEnabled()
	if ModManager == nil or KnownModIndex == nil then
		TheLog.ch.Mods:print("AreAnyModsEnabled returning false because ModManager and KnownModIndex hasn't been created yet.")
		return false
	end
...
function AreClientModsDisabled()
	if KnownModIndex == nil then
		TheLog.ch.Mods:print("AreClientModsDisabled returning false because KnownModIndex hasn't been created yet.")
		return false
	end
...
function CreateEnvironment(modname, isworldgen)
	...
	env.modimport = function(modulename)
		TheLog.ch.Mods:print("modimport: " .. env.MODROOT .. modulename)
...
function ModWrangler:LoadServerModsFile()
	...
	if fn ~= nil then
		local mods_err_fn = function(err)
			...
			TheLog.ch.Mods:print("#ERROR: Failure to load dedicated_server_mods_setup.lua:", err)
			TheLog.ch.Mods:print("#Shutting down")
			TheLog.ch.Mods:print("########################################################")
			RequestShutdown(ExitCode.ModServerFailure)
		end
...
function ModWrangler:FrontendLoadMod(modname)
	...
	if not KnownModIndex:DoesModExistAnyVersion(modname) then
		TheLog.ch.Mods:print(string.format("Tried frontend-loading mod '%s' but it doesn't exist.", modname))
		return
	end
	TheLog.ch.Mods:print("FrontendLoadMod", modname)
...
function ModWrangler:FrontendUnloadMod(modname)
	TheLog.ch.Mods:print(string.format("Frontend-Unloading mod '%s'.", modname or "all"))
...
function ModWrangler:LoadMods(worldgen)
	if not MODS_ENABLED or not GAMEPLAY_MODS_ENABLED then
		return
	end
...
function ModWrangler:InitializeModMain(modname, env, mainfile, safe)
	...
	TheLog.ch.Mods:print("Mod: " .. ModInfoname(modname), "Loading " .. mainfile)
	...
	if type(fn) == "string" then
		...
		return false
	elseif not fn then
		TheLog.ch.Mods:print("Mod: " .. ModInfoname(modname), "  Mod had no " .. mainfile .. ". Skipping.")
		return true
	else
		local status = nil
		local r = nil
		if safe then
			status, r = RunInEnvironment_Verbose(fn, env)
		else
...
-- TODO(mods): probably would be good to support this. We need a mod menu first.
function ModWrangler:DisplayBadMods()
	...
	if #self.failedmods > 0 then
		for i, failedmod in ipairs(self.failedmods) do
			...
			TheLog.ch.Mods:print("Disabling " .. ModInfoname(failedmod.name) .. " because it had an error.")
		end
	end
	...
	if TheFrontEnd then
		for k, badmod in ipairs(self.failedmods) do
			SetGlobalErrorWidget(
				...
				STRINGS.UI.MAINSCREEN.SCRIPTERROR_MOD.BLAME_TARGET
					.. "\n"
				...
				{
					{
						...
						cb = function()
							TheSim:ForceAbort(ExitCode.BadMod)
						end,
					},
					{
						text = STRINGS.UI.MAINSCREEN.SCRIPTERROR_MOD.RESET_WITHOUT_MODS:subfmt({
				...
				ANCHOR_LEFT,
				STRINGS.UI.MAINSCREEN.SCRIPTERROR_MOD.BLAME_RESOLVE,
				20
			)
		end
		self.failedmods = {}
...
function ModWrangler:RegisterPrefabs()
	...
	for i, modname in ipairs(self.enabledmods) do
		...
		TheLog.ch.Mods:print("Mod: " .. ModInfoname(mod.modname), "Registering prefabs")
		...
		if mod.PrefabFiles then
			for i, prefab_path in ipairs(mod.PrefabFiles) do
				TheLog.ch.Mods:print("Mod: " .. ModInfoname(mod.modname), "  Registering prefab file: prefabs/" .. prefab_path)
				local ret = runmodfn(mod.LoadPrefabFile, mod, "LoadPrefabFile")("prefabs/" .. prefab_path)
				if ret then
					for i, prefab in ipairs(ret) do
						TheLog.ch.Mods:print("Mod: " .. ModInfoname(mod.modname), "    " .. prefab.name)
		...
		TheLog.ch.Mods:print("Mod: " .. ModInfoname(mod.modname), "  Registering default mod prefab")
...
function ModWrangler:UnloadPrefabsFromData(data)
	for i, v in ipairs(data) do
		TheLog.ch.Mods:print("unloading prefabs for mod " .. v.infoname)
...
function ModWrangler:SetPostEnv()
	...
	-- TODO(mods): These strings are all wrong.
	--TheLog.ch.Mods:print("\n\n---END MOD INFO SCREEN---\n\n")
	...
	if
		(modnames ~= "" or newmodnames ~= "" or failedmodnames ~= "" or forcemodnames ~= "")
		and TheSim:ShouldWarnModsLoaded()
	then
		--if (#self.enabledmods > 0)  and TheSim:ShouldWarnModsLoaded() then
		if not DISABLE_MOD_WARNING and IsInFrontEnd() then
			TheFrontEnd:PushScreen(ModWarningScreen(STRINGS.UI.MAINSCREEN.MODTITLE, moddetail, {
				{
					text = STRINGS.UI.MAINSCREEN.TESTINGYES,
					cb = function()
	...
	elseif KnownModIndex:WasLoadBad() then
		TheFrontEnd:PushScreen(ModWarningScreen(STRINGS.UI.MAINSCREEN.MODSBADTITLE, STRINGS.UI.MAINSCREEN.MODSBADLOAD, {
			{
				text = STRINGS.UI.MAINSCREEN.TESTINGYES,
				cb = function()
					TheFrontEnd:PopScreen()
		...
		if mod.postinitfns[type] then
			...
			if modfns ~= nil then
				for i, modfn in ipairs(modfns) do
					--TheLog.ch.Mods:print(modname, "added modfn "..type.." for "..tostring(id))
...
function ModWrangler:GetPostInitData(type, id)
	...
	for i, modname in ipairs(self.enabledmods) do
		...
		if mod.postinitdata[type] then
			...
			if data ~= nil then
				--TheLog.ch.Mods:print(modname, "added moddata "..type.." for "..tostring(id))
	...
	for i, mod_name in ipairs(ModManager:GetEnabledServerModNames()) do
		if mod_name:len() > 0 then
			...
			if modinfo.all_clients_require_mod then
				--TheLog.ch.Mods:print("adding mod to verify", mod_name)
...
function ModWrangler:GetLinkForMod(mod_name)
	...
	if Platform.IsRail() then
		...
		if is_known and KnownModIndex:IsWorkshopMod(mod_name) then
		...
		elseif KnownModIndex:IsWorkshopMod(mod_name) then
			url = "http://steamcommunity.com/sharedfiles/filedetails/?id="
				.. KnownModIndex:GetWorkshopIdNumber(mod_name)
		else
