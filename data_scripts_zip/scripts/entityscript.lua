...
--- @field OnRemoveEntity function?
--- @field OnSave function?
--- @field OnSetSpawnInstigator function?
EntityScript = Class(function(self, entity)
	self.entity = entity
	self.components = {}
	self.netcomponents = {}
	self.nrnetcomponents = 0	-- Ugh lua, why can't you just give me the nr of pairs in the table. -- Try table.numkeys()!
...
function EntityScript:CanTakeControl(attack)
	...
	if attack and health_cmp and self.components.combat then
		...
		if self:IsAlive()
			and damage >= health_cmp:GetCurrent()
			and not self:HasTag("no_state_transition")
		then
			return true
		end
	end
...
function EntityScript:GetTimeInLimbo()
	if self:IsInLimbo() then
		if not self._timeinlimbo then
			self._timeinlimbo = GetTime()
		end
		return GetTime() - self._timeinlimbo
	end
end
...
-- How long since entity spawned in seconds.
...
function EntityScript:SetBrain(name, data)
	if self.brain ~= nil then
...
--- @param rng table?  Optional krandom-based rng
function EntityScript:GetRandomEntityByTagInRange(range, tags, isalive, checkinvisible, rng)
	local x, z = self.Transform:GetWorldXZ()
	...
	for i, ent in ipairs(ents) do
		if (isalive == nil or isalive == ent:IsAlive())
			and (checkinvisible or ent:IsVisible())
			and ent ~= self
		then
	...
	if #valid_ents > 0 then
		if rng then
			return valid_ents[rng:Integer(#valid_ents)]
		else
...
local function CheckUnpaused()
	local is_paused, reason = IsGameplayPaused()
	local is_forbidden = is_paused
		-- InitGame is a known case so ignore it.
		and reason ~= "InitGame"
		-- Ignore failures during error pause.
		and not TheFrontEnd.cachedError
	if is_forbidden then
		TheLog.ch.Task:trace_at(2, "Task added while paused won't proceed until SetGameplayPause(false). Pause reason:", reason)
	end
	-- Not yet confident enough to assert.
	--~ return not is_forbidden
	return true
end

function EntityScript:AddTaskInternal(task)
	dbassert(CheckUnpaused(), "Starting Task while paused.")
...
function EntityScript:Remove(forceremove)
	...
	self._entity_remove_state = "pushing onremove"
	self:PushEvent("onremove")

	self._entity_remove_state = "calling OnRemoveEntity"
	for k, v in pairs(self.components) do
	...
	self._entity_remove_state = "removed"
